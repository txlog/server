...
func PostLogin(oidcService *auth.OIDCService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			slog.Error("Failed to generate state: " + err.Error())
...
func GetCallback(oidcService *auth.OIDCService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil || storedState != state {
			slog.Error("State parameter mismatch")
		...
		if err != nil {
			slog.Error("Failed to exchange code for tokens: " + err.Error())
		...
		if !ok {
			slog.Error("ID token is missing")
		...
		if err != nil {
			slog.Error("Failed to verify ID token: " + err.Error())
		...
		if err != nil {
			slog.Error("Failed to create/update user: " + err.Error())
			c.Redirect(http.StatusSeeOther, "/login?error=user_creation_failed")
			return
		}

		if !user.IsActive {
			slog.Warn("User " + user.Email + " is not active")
		...
		if err != nil {
			slog.Error("Failed to create user session: " + err.Error())
		...
		slog.Info("User " + user.Email + " logged in successfully")
...
func PostLDAPLogin(ldapService *auth.LDAPService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			slog.Error("LDAP authentication failed: " + err.Error())
		...
		// Set session cookie
		c.SetCookie("session_id", sessionID, 7*24*3600, "/", "", isSecureCookie(), true)
		slog.Info("User " + user.Email + " logged in successfully via LDAP")
		c.Redirect(http.StatusSeeOther, "/")
	}
}
		...
		if err == nil && sessionID != "" {
			...
			if oidcService != nil {
				if err := oidcService.InvalidateUserSession(sessionID); err != nil {
					slog.Error("Failed to invalidate user session: " + err.Error())
				}
