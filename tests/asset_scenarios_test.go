...
func TestAssetReactivation(t *testing.T) {
	...
	t.Run("Create asset with hostname1", func(t *testing.T) {
		...
		err := am.UpsertAsset(tx, hostname1, machineID, time.Now(), sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
	...
	t.Run("Move asset to hostname2", func(t *testing.T) {
		...
		err := am.UpsertAsset(tx, hostname2, machineID, time.Now(), sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
...
func TestAssetHistoryPreservation(t *testing.T) {
	...
	for _, m := range machines {
		tx, _ := db.Begin()
		err := am.UpsertAsset(tx, hostname, m.id, m.timestamp, sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
			tx.Rollback()
...
func TestAssetDatabaseConstraints(t *testing.T) {
	...
	t.Run("Cannot have multiple active assets per hostname", func(t *testing.T) {
		...
		am := models.NewAssetManager(db)
		tx, _ := db.Begin()
		err := am.UpsertAsset(tx, hostname, machineID1, time.Now(), sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
			tx.Rollback()
		...
		tx, _ = db.Begin()
		err = am.UpsertAsset(tx, hostname, machineID2, time.Now(), sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
			tx.Rollback()
...
func TestAssetTimestampEdgeCases(t *testing.T) {
	...
	t.Run("Update with older timestamp", func(t *testing.T) {
		...
		tx, _ := db.Begin()
		now := time.Now()
		err := am.UpsertAsset(tx, hostname, machineID, now, sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
			tx.Rollback()
		...
		err = am.UpsertAsset(tx, hostname, machineID, older, sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
			tx.Rollback()
