...
func TestFullAssetLifecycle(t *testing.T) {
	...
	t.Run("Create initial asset", func(t *testing.T) {
		...
		timestamp := time.Now()
		err := am.UpsertAsset(tx, hostname, machineID1, timestamp, sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
	...
	t.Run("Replace asset with new machine_id", func(t *testing.T) {
		...
		timestamp := time.Now()
		err := am.UpsertAsset(tx, hostname, machineID2, timestamp, sql.NullBool{}, sql.NullString{}, "", "", sql.NullBool{})
		if err != nil {
